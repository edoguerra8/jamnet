...
export default function LibraryPage() {
  ...
  const totalGenreTracks = Object.values(genrePlaylists).reduce((n, t) => n + t.length, 0)
  ...
  const handleRenameStart = (comp: Compilation) => {
  ...
  const handleRenameCommit = () => {
    if (renamingId && renameValue.trim()) {
      renameCompilation(renamingId, renameValue.trim())
    }
    setRenamingId(null)
    setRenameValue('')
    reload()
  ...
  const handleRemoveFromGenre = (trackId: string, genre: string) => {
    removeFromGenrePlaylist(trackId, genre)
    reload()
  }

  const handleRemoveFromCompilation = (trackId: string, compilationId: string) => {
    removeTrackFromCompilation(trackId, compilationId)
    reload()
  }
  ...
  return (
    <main className="min-h-dvh bg-ivory px-5 pt-safe pb-safe">
      ...
      {totalGenreTracks === 0 && compilations.length === 0 ? (
                  ...
                  return (
                    <div key={genre} className="flex flex-col">
                      <button
                        ...
                        onClick={() => setExpanded(isOpen ? null : `genre-${genre}`)}
                      >
                        <PlaylistCover genre={genre} className="w-12 h-12 shrink-0" />
                        <div className="flex flex-col flex-1 min-w-0">
                          <span className="font-sans text-[15px]">{genre}</span>
                            ...
                            <div className="flex flex-col gap-0 pb-2">
                              {tracks.map(t => (
                                <div key={t.id} className="flex items-center gap-3 py-2.5">
                                  ...
                                  <button
                                    onClick={() => handleRemoveFromGenre(t.id, genre)}
                                    className="p-1.5 opacity-30 hover:opacity-80 transition-opacity shrink-0"
                                    aria-label="Remove"
                                  >
          ...
          <section className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[12px] font-sans text-terracotta flex items-center gap-1"
              >
            ...
            {compilations.length === 0 ? (
              <p className="text-[13px] font-sans text-muted">
                Tap the bookmark icon in the flow to add tracks to a compilation.
              </p>
            ) : (
              <div className="flex flex-col gap-2">
                    ...
                    <div key={comp.id} className="flex flex-col">
                      <div className="flex items-center gap-4 py-3 w-full">
                        <button
                          ...
                          onClick={() => setExpanded(isOpen ? null : `comp-${comp.id}`)}
                        >
                          ...
                          <div className="flex flex-col flex-1 min-w-0">
                            {renamingId === comp.id ? (
                              ...
                              <span
                                ...
                                onDoubleClick={(e) => { e.stopPropagation(); handleRenameStart(comp) }}
                              >
                                {comp.name}
                              </span>
                            )}
                            <span className="text-[12px] font-sans text-muted">
                              {comp.tracks.length} {comp.tracks.length === 1 ? 'track' : 'tracks'}
                            </span>
                          </div>
                        </button>
                        <div className="flex items-center gap-3 shrink-0">
                          <button
                            onClick={() => handleRenameStart(comp)}
                          ...
                          <button
                            onClick={() => handleRemoveCompilation(comp.id)}
                            className="opacity-30 hover:opacity-80 transition-opacity"
                            aria-label="Delete compilation"
                          >
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
                              <polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6M14 11v6" strokeLinecap="round" />
                            </svg>
                          </button>
                          <svg
                            viewBox="0 0 24 24" width="16" height="16" fill="none"
                            ...
                            <div className="flex flex-col gap-0 pb-2">
                              {comp.tracks.length === 0 ? (
                                ...
                                <div key={t.id} className="flex items-center gap-3 py-2.5">
                                  ...
                                  <button
                                    onClick={() => handleRemoveFromCompilation(t.id, comp.id)}
                                    className="p-1.5 opacity-30 hover:opacity-80 transition-opacity shrink-0"
                                    aria-label="Remove"
                                  >
                                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
